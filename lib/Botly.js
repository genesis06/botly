...
Botly.prototype.requestThreadControl = function (options, callback){
    request.post(
        {
            url: FB_URL+"me/pass_thread_control",
            json: true,
            qs: {
...
Botly.prototype.handleMessage = function (req) {
    ...
    body.entry.forEach(entry => {
        if(entry.standby){
        ...
        else if (entry.messaging && entry.messaging.length) {
          entry.messaging.forEach(message => {
            ...
            _handleRequestThreadControl.call(this,message);
          }, this);
        }
...
function _handleRequestThreadControl(message) {
    var value;
    if (message.request_thread_control) {
        value = { requested_owner_app_id: message.request_thread_control.requested_owner_app_id, metadata: null };
    }

    if (value) {
        this.emit('request_thread_control', message.sender.id, message, value);
    }
}
